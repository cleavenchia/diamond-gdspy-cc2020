...
from devices_zep import dev_clp_phc_tsptets_twg_zep as omc
...
gdsname = '20200102rectOMCZEPtest.gds'
...
""" geometry definitions
    ...
    tets_side: add tethers on top, bottom, or both halves of beam
...
zepbox_wid = 10
geom0 = {'gdslib':gdslib,
         'wg_wid':wg_wid,      'clp_wid':5,                 'clp_strlen':0,
         'clp_taplen':0,       'clp_outlen':2.5,            'out_len':2.5,
         'tapsup_len':12.5,    'tapsup_wid': wg_wid*1.3,    'ctr_len':0,
         'in_wid':wg_wid,      'in_len':0,                  'end_len':40,
         'end_wid':0.1,        'wid_corr':0.2,              'phc_scl':1.02,
         'n_tets':3,           'w_tets':0.2,                'g_tets':0.5,
         'tets_side':0,        'l_tets':zepbox_wid,
         'zepbox_wid':zepbox_wid,
...
# device variations
nmirR_all = np.array([4])  #from min, min+1, ..., max-1
phc_scl_all = np.array([1.00,1.02])
...
# array parameters
col_sp = np.ceil(1.5*maxlen/5.0)*5.0    # spacing between columns, ceil to nearest 5um
...
for nri,nr in enumerate(nmirR_all):
    ...
    mirlbl1 = gp.Text('M'+str(int(nr)),15,
                      (dev_xmin-col_sp,-2*row_offset+(nri*nreps)*row_sp-yctr),**lablspec) 
...
for psi,ps in enumerate(phc_scl_all):
    ...
    scllbl1 = gp.Text('S'+str(int(np.mod(ps*100,10))),15,
                      ((psi-1)*col_sp, 20+(nrows*nreps-1)*row_sp - yctr+row_offset),
                      **lablspec) 
