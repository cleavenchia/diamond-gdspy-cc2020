...
from devices import dev_clp_phc_tsp_twg as omc, cantilever as cant
...
gdsname = '20200104tri45oIBEtest.gds'
...
geom0 = {'gdslib':gdslib,
         ...
         'end_wid':0.1,        'wid_corr':0.6,              'phc_scl':1.02,
         'a':0.566,         'hx':0.226,     'hy':0.699,     'maxdef':0.15906,
         'oblong':3.0563,   'holeinctr':1,  'taper_func':'cubic',
         'ndef':9,          'nmirL':10,     'nmirR':3,      'nwgm':5,
         'aW':0.435,        'hxW':0.179,    'hyW':0.272}

# device variations
nmirR_all = np.array([2,3])  #from min, min+1, ..., max-1
phc_scl_all = np.array([0.98,1.00,1.02])
cant_wid = np.array([1.0,1.2,1.4])
...
nreps = 2                           # no. of replicates for each device
ncols = phc_scl_all.shape[0]
nrows = nmirR_all.shape[0]
yctr = np.ceil(0.5*(nrows*nreps-1))*row_sp
...
for psi,ps in enumerate(phc_scl_all):
    ...
    devarr.add(gp.CellReference(pscell,((psi-1)*col_sp,(psi-2)*row_offset)))
    
    # add cantilevers below devices
    outcant = cant('cant'+str(int(cant_wid[psi]*1000)),gdslib,
                   wg_wid=cant_wid[psi],beam_len=5,clp_wid=5,clp_strlen=5,
                   clp_taplen=3,wid_corr=0.6)
    cant_box = outcant['box']
    cant_len = cant_box[1,0]-cant_box[0,0]
    [devarr.add(gp.CellReference(outcant['cell'],
                                 ((psi-1)*col_sp+50+rep*(cant_len+15),
                                  45+(nrows*nreps-1)*row_sp - yctr+row_offset)))
        for rep in np.arange(nreps)]
    [devarr.add(gp.CellReference(outcant['cell'],
                                 ((psi-1)*col_sp+(rep+2.5)*20,
                                  25+(nrows*nreps-1)*row_sp-yctr),
                                  rotation=-90))
        for rep in np.arange(nreps)]
#    [devarr.add(gp.CellReference(outcant['cell'],
#                                 ((psi-1)*col_sp+rep*(cant_len+20),
#                                  -yctr-(rep+nreps)*row_sp+row_offset)))
#        for rep in np.arange(nreps)]
...
for nri,nr in enumerate(nmirR_all):
    ...
    mirlbl1 = gp.Text('M'+str(int(nr)),15,
                      (dev_xmin-1*col_sp,-2*row_offset+(nri*nreps)*row_sp-yctr),**lablspec) 
...
for psi,ps in enumerate(phc_scl_all):
    ...
    scllbl1 = gp.Text('S'+str(int(np.mod(ps*100,10))),15,
                      ((psi-1)*col_sp, 10+(nrows*nreps-1)*row_sp - yctr+row_offset),
                      **lablspec) 
...
devarr.add(gp.CellArray(mks['cell'],2,2,(400,200),(-200,-100)))
